...
import type { OutcomeMetric } from '@/types/database';

type EditableOutcome = { id?: string; label: string };

export default function EditOutcomesScreen() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        setItems(
          (data as OutcomeMetric[] | null)?.map((o) => ({ id: o.id, label: o.label })) ?? [],
        );
        setLoading(false);
      });
  }, [user?.id]);
